...
var network = {};
function longRoadNetwork(){
	for(var x = 0; x < world.tiles.length; x++){
		for(var y = 0; y < world.tiles[x].length; y++){
			var tile = world.tiles[x][y];
			if(tile.road && !tile.network && !tile.crossing){
				continueRoad(tile);	
				console.log("Network done, analysing...");
				$.each(network, function(i, n){
					if(!n) return true;
					n.neighbours = [];
					for(var k = 0; k < n.length; k++){
						var neighbour = n[k].start;
						if(!neighbour || neighbour == i){
							if(!n[k].end || n[k].end == i)
								continue;
							neighbour = n[k].end;
						}
						n.neighbours.push(neighbour);
					}
				});
			}
		}
...
function continueRoad(from, previous, road){
	++counter;
	...
	for(var i = 0; i < neighbours.length; i++){
		var cx = neighbours[i][0];
		var cy = neighbours[i][1];
		
		var neighbour = lookUpCoord.apply(undefined, neighbours[i]);
		if(neighbour == previous)
			continue; // already visited

		if(neighbour.road){
			if(network[cy * world.sizeX + cx]){
				road.end = cy * world.sizeX + cx;
				network[cy * world.sizeX + cx].push(road);
				continue;
			} // is a crossing
			if(contains(neighbour, road))
				continue;
			var newNeighbours = false;
			for(var n in neighbour.neighbours){
				if(!neighbour.neighbours[n] || neighbour.neighbours[n].length != 2){
					continue;
				}
				
				var t = lookUpCoord.apply(undefined, neighbour.neighbours[n]);
				if(t.road && t.globalPosition != from.globalPosition){
					if(!contains(neighbour.neighbours[n], neighbours)){
						newNeighbours = true;
						break;
					}
				}
	...
	if(candidates.length == 0){
		...
		road.push(candidates[0]);
		from.network = true;
		return continueRoad(candidates[0], from, road);
	} else {
		road.end = from.globalPosition.y * world.sizeX + from.globalPosition.x;
		var connections = [road];
		network[from.globalPosition.y * world.sizeX + from.globalPosition.x] = connections;
		from.crossing = true;
		for(var i = 0; i < candidates.length; i++){
			var newRoad = [candidates[i]];
			newRoad.start = from.globalPosition.y * world.sizeX + from.globalPosition.x;
			continueRoad(candidates[i], from, newRoad);
			connections.push(newRoad);
		}
	}
}
...
/*TODO 
		...
		THIS TEXT IS OVER A YEAR OLD: MOST NEW CONTENT IN OTHER FILES 
...
*/
function mountainColor(tile, season){
	var r,g,b;
	var c = 150 + tile.depth*14;
	var y = Math.abs(world.sizeY / 2 - tile.globalPosition.y);
	var percent = y / (world.sizeY / 2);

	var rS = 2 - 0.8 * percent;
	var bS = 1 + 0.3 * percent;
	...
	function mountainColor(tile, season){
		var r,g,b;
		var c = 150 + tile.depth*16;
		var y = Math.abs(world.sizeY / 2 - tile.globalPosition.y);
		var percent = y / (world.sizeY / 2);

		var rS = 1.2 - 0.1 * percent;
		var bS = 0.9 + 0.4 * percent;
...
var sandColor = {r:255,g:228,b:156};
...
function getColorForTile(tile){
	...
	if(type == "water"){
		...
		if(seasonT == "Winter"){
			...
			r = Math.round(10+rS*c); //should vary between 1.2 and 2
			g = Math.round((133+c/2));
			b = (10+bS*c); //should vary between 1.3 and 1
		
			if(tile.resources){
/*				if(tile.resources.rice){
					b += 100;
					g -= 80;
					r -= 100;	
				} else if(tile.resources.wheat){
					r += 80;
					g += 70;
					b -= 50;
				}*/
		...
		var rgb;
		rgb = mountainColor(tile, seasonT);
		r = rgb.r;
		g = rgb.g;
		b = rgb.b;
	}
...
/*		r = 255;
		g = 228;
		b = 156; */
		return {"r":plusMinus(sandColor.r,3),"b":plusMinus(sandColor.b,3),"g":plusMinus(sandColor.g,3)};	
		//return "#FF9";	
	}
	else if(type == "river"){
...
function updateTileInfo(tile)
{
	...
	try{	
		...
		if(area.type == "settlement"){
			...
			//	value = city.value;
			id = city.id;
			$("#tileValue").text(city.name).click(function(){
				showCityScreen(cityScreen(id));
			}).addClass("link");
			city.showRange.show();
			selectedCity = city;
		} else {
			selectedCity = undefined;
			$("#tileValue").text(id + ". Tile Value: " + value).removeClass("link");
		}
	}
	catch(e){
		// field undefined or null or something
	}
	$("#tilePopulation").text("Population: " + population);
	$("#tileActions").empty();
	//fillActionMenu(tile);
...
function changeOverlayCanvas(func)
{ 
    overlayFunc = func;
    overlayActive = true;
    updateOverlayCanvas();
...
var drawCultureTriangle = function(cctx){
	if(!selectedCity) return;
	var culture = world.cultures[selectedCity.cultureID];
	var points = [];
	$.each(culture, function(i, c){
		points.push(c.center);
	});

	var polygon = jarvis(points);
	
	cctx.beginPath();
	cctx.fillStyle = "rgba(0,0,0,0.2)";
	cctx.moveTo(polygon[0].x*scale - xScroll, polygon[0].y*scale - yScroll);
	$.each(polygon, function(i, p){
		if(i==0) return true;
		cctx.lineTo(p.x*scale - xScroll, p.y*scale - yScroll);
	});
	cctx.fill();
	cctx.closePath();
};
...
function updateOverlayCanvas(){
    ...
    $.each(overlayFunctions, function(i,fn){
    	fn(cctx);
    });
...
function drawSingleTile(cctx, x, y, fillColor, strokeColor){
	...
	if(fog){
		sourceX = world.dynasty.root.location.x;
		sourceY = world.dynasty.root.location.y
    ...
    var zoomCutOff = 3;
	if((!fog || inrange) && (world.tiles[x][y].type == "settlement" || zoomLevel > zoomCutOff) && world.tiles[x][y].borders != null){
		var city = world.areas[tile.field];
		if(!city.walled && zoomLevel < zoomCutOff)
			return;
		cctx.fillStyle = "gray";
		var borderSize = 2;
		if(world.tiles[x][y].type != "settlement"){
			borderSize = 1;
			cctx.fillStyle = color;
		}
		...
		for(var i = 0; i < world.tiles[x][y].borders.length; i++){
			...
			if(border == "left"){
				cctx.fillRect(xStart,yStart,borderSize,tileHeight);
			}
			else if(border == "right"){
				cctx.fillRect(xStart+tileWidth-borderSize,yStart,borderSize,tileHeight);
			}
			else if(border == "above"){
				cctx.fillRect(xStart,yStart,tileWidth,borderSize);
			}
			else if(border == "below"){
				cctx.fillRect(xStart, yStart+tileHeight-borderSize,tileWidth,borderSize);
			}
		}
	}
...
function sketchProc(processing) {
    ...
    function drawTile(tile) {
		...
		if(x < 0){ tileWidth += x; x = 0; }
		if(y < 0){ tileHeight += y; y = 0; }
		if(xEnd > w*2){ tileWidth -= xEnd-w; }
		if(yEnd > h*2){ tileHeight -= yEnd-h; }
		...
		processing.rect(x,y,tileWidth,tileHeight);
		//drawImage
		//cctx.strokeRect(x+2,y+2,tileWidth-2, tileHeight-2);
    }
...
function zoomInOn(tile,event){
	var xShift, yShift;
	if(!event)
		event = zoomLevel * scale;

	if(event.pageX == undefined){
		zoomLevel = Math.round(event/scale);
		zoomState = true;
		scale = Math.round(event);
		var xShift = Math.round(w/2);
		var yShift = Math.round(h/2);
	}
	else{
...
function keyBindings(){
	...
	$("#OptionsHeader").live(uiClick, function(event){
		exportAsImage();
	});
}
...
function continueGame(){
	alert("Yeah... this doesn't work at the moment. Sorry!");
...
function openInitMenu(createWorld){
	...
	var downloadBtn = $("<div>").addClass("selectable").attr("id", "download").click(exportAsImage).text("Download map as .png");
	$("#NewGameMenu").append(quickStart).append(newGameBtn).append(loadGameBtn).append(downloadBtn);
	if(saveExists()){
	...
	var locationSet = $("<div>").addClass("selectable").attr("id","lSet").text("Set Starting Location").click( function(){
		...
		$("#newPlayer").click( function(e){
			...
			if(loc.type == "settlement"){
				world.dynasty.root.location = loc.globalPosition;
...
function editWorld(location){
	return alert("Nope. Not done yet :(");
...
function newGame(){
	...
	$("#NewGameMenu").empty().append(setSeed).append(seedInput).append(seedTime).append(seedConfirm).append(seedDiscard)
			.append(tilesizeInput).append(tilesize).append(regenerate).append(edit).append(downloadBtn).append(charGen).append(back);
}
...
function selectMode(mode){
	...
	var dynasty = $("<div>").addClass("selectable").text("Start").click( function(){mode = "dynasty";//loadJSFile("scripts/dynasty.js"); 
		openInitMenu();})
		.hover(function(){$("#bgImage2").show(); $("#bgImage1").hide();});
	$("#NewGameMenu").empty().append(dynasty);
}
...
function start(){
	...
	zoomInOn(world.tiles[pos.x][pos.y], 32);
	//fog = true;
	range = 20;
	//$("#Loading").show();
	keyBindings();
