...
function Person(location, family, gender, fname, sname, id, age, DoB, traits, faction){
	...
	if(family && !sname) sname = family.father.sname;
...
function generateTraits(person, family){
	...
	var m = (person.gender == "Male");
	var f = family != undefined && family.mother != null && family.father != null; 

	$.each(traitsData, function(trait, data){
		var chance = random(1000000);
		if(f){	
			if(family.mother.traits[trait] && family.mother.traits[trait].count > 0)
				chance -= 250000 // 25%
			if(family.father.traits[trait] && family.father.traits[trait].count > 0)
				chance -= 250000 // 25%
		}
		if(m){
			if(chance <= 1000000*data.maleRandomChance){
				incrementTrait(person, trait);
			}
	
		}
		else{
			if(chance <= 1000000*data.femaleRandomChance){
				incrementTrait(person, trait);
			}
		}
	});
...
function generateSurName(){
	...
	return messWithString(surnames[r]);
}
	
function generateFirstName(gender){
	 if(gender == "Male"){
		var r = random(0,maleNames.length-1);
		return messWithString(maleNames[r]);
	 }
	 else{
		var r = random(0,femaleNames.length-1);
		var s = messWithString(femaleNames[r]);
		if(s[s.length-1] != "a") s += "a";
		return s;
	 }
}
...
function countTraits(array){
	...
	$.each(results, function(i,val){
		percentage = val / array.length;
		var globalChance = (traitsData[i].maleRandomChance + traitsData[i].femaleRandomChance) / 2;
		var comparison =  (percentage < globalChance) ? " < " : " > ";
		comparison = percentage + comparison + globalChance;
		results[i] = [results[i], percentage, comparison];
	});
	return results;
}
...
function generatePopulation(){
	...
	var places = Math.round(Math.max(16, Math.min(70, size * (random(2,10) / 20000))));
