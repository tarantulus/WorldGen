...
function distanceBetween(start, neighbour){
    ...
    var value = neighbour.value - start.value;

    if(value < 0) multiplier++; 

    return g*multiplier + random(0,2);
}
...
function findPath(start, goal){
    if(!start.x)
        start = start.globalPosition;
    if(!goal.x)
        goal = goal.globalPosition;
    var goalCond;
    var distanceFunc;
    var heuristicFunc;
    var returnFunc;
    ...
    returnFunc = function(end){
        var path = [end.globalPosition];
        while(end.cameFrom !== undefined){
            path.push(end.cameFrom.globalPosition);
            end = end.cameFrom;
            if(end == start) break;
        }
